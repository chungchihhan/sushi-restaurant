...
import { getShop, editShop, UploadImageForShop ,GetImageForShop , updateMeal , getMealsByShopId ,deleteMeal} from "../../../utils/client";
...
export default function ShopEditPage() {
  ...
  const handleImageUpload = async (imageFile: File) => {
    ...
    if (shopId) {
      try {
        ...
        // Upload image
        const uploadResponse = await UploadImageForShop(shopId, formData);
  ...
  const updateMealData = async (updatedMeal:UpdatedMealData,mealId:string) => {
    try {
      const shopId = localStorage.getItem("shopId")||"";
  ...
  return (
    ...
    <div className="mx-auto max-w-2xl rounded-lg bg-gray-300 p-8 shadow-lg">
      ...
      {uploadedImageUrl && (
        <div className="mb-4">
          <img src={uploadedImageUrl} alt="Uploaded Shop Image" className="w-full h-auto" />
        </div>
      )}
      <input
        type="file"
    ...
    <div className="flex felx-col items-center gap-4 p-4">
      ...
      <div>
        ...
        <MealCreateModal isOpen={isModalOpen} onRequestClose={() => setIsModalOpen(false)} onMealCreated={fetchMeals} />
      </div>
    </div>
